"""Build and save ScaNN index from LangChain or LangGraph documentation.

This module provides functionality to build a ScaNN vector index from documentation
...
To force a rebuild: Delete the scann_index directory.
...
# Initialize Ray
ray.init(include_dashboard=True, dashboard_host="0.0.0.0", num_cpus=4)
...
def embed_chunks_with_progress(
    ...
    """Embed a batch of document chunks and create a ScaNN index.
    ...
    Returns:
        ScaNN index containing the embedded chunks.
    """
    print(f"[Batch {batch_id}] Starting embedding of {len(chunks)} chunks")
    embeddings = HuggingFaceEmbeddings(model_name=model_name)
    result = ScaNN.from_documents(chunks, embeddings)
...
def build_index(
    ...
    model_name: str = "sentence-transformers/all-MiniLM-L6-v2",
    index_dir: str = "scann_index",
    checkpoint_dir: str = "embedding_checkpoints"
) -> ScaNN:
    """Build and save a ScaNN index from documentation website.
    
    This function loads documentation from a website, preprocesses it into chunks,
    embeds the chunks using a specified model, and saves the resulting ScaNN index.
    ...
    Args:
        ...
        index_dir: Directory to save the final ScaNN index.
        checkpoint_dir: Directory to save intermediate checkpoints.
        
    Returns:
        The constructed ScaNN index.
    ...
    # Check if ScaNN index already exists
    index_file = os.path.join(index_dir, "index.scann")
    if os.path.exists(index_file):
        print(f"Loading existing ScaNN index from '{index_dir}'...")
        embeddings = HuggingFaceEmbeddings(model_name=model_name)
        index = ScaNN.load_local(index_dir, embeddings, allow_dangerous_deserialization=True)
...
if __name__ == "__main__":
    ...
    print(f"\nIndex building complete! Saved to 'scann_index' directory")
